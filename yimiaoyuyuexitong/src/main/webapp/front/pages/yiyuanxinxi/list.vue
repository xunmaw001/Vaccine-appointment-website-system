<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yiyuanxinxiName" type="text" placeholder="医院名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in yiyuanxinxiTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.yiyuanxinxiPhoto?product.yiyuanxinxiPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.yiyuanxinxiName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('yiyuanxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('yiyuanxinxi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('yiyuanxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 医院类型 字段 字典表查询方法
		  	let yiyuanxinxiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'yiyuanxinxi_types'
		  	});
		  	this.yiyuanxinxiTypesList = yiyuanxinxiTypes.data.list;
	   		//当前表的 地区 字段 字典表查询方法
		  	let addressTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'address_types'
		  	});
		  	this.addressTypesList = addressTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.yiyuanxinxiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.yiyuanxinxiTypes = this.categoryName
                            }
			...
			async search() {
				...
				if (this.searchForm.yiyuanxinxiName) {
                    searchForm['yiyuanxinxiName'] = this.searchForm.yiyuanxinxiName
                }
				if (this.searchForm.yiyuanxinxiName) {
					searchForm['yiyuanxinxiName'] = this.searchForm.yiyuanxinxiName
				}
