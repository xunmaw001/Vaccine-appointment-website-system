...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.yiyuanxinxiName}}
						</view>
				</view>
   	 <view style="margin-top: 10rpx;"></view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.yiyuanxinxiContent"></rich-text>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				// 主图
				this.swiperList = this.detail.yiyuanxinxiPhoto ? this.detail.yiyuanxinxiPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('yiyuanxinxiLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					yiyuanxinxiId: this.id
				});
				// 如果是第一页数据置空
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../yiyuanxinxiLiuyan/add-or-update?yiyuanxinxiId=${this.id}`)
            },
		}
	}
</script>
